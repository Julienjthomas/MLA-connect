...
The `AuthController` SHALL subscribe to `auth.onAuthStateChange` to refresh `user` on `signedIn` and clear it on `signedOut`.

#### Scenario: External sign-out
- **WHEN** the Supabase client emits `AuthChangeEvent.signedOut`
- **THEN** `user.value` becomes null

### Requirement: Logout clears session and routes to welcome
`logout()` SHALL call `auth.signOut()`, null the cached user, and navigate to the welcome route.

#### Scenario: User taps Logout
- **WHEN** the user confirms the logout dialog on the profile tab
- **THEN** the session is signed out and the welcome route is shown via `Get.offAllNamed`
